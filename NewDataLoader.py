...
class DataLoader():
    ...
    '''
    def next_morning_batch(self, seq_len = 30, batch_size = 32):
    ...
    Get Morning data
    '''
    def get_morning_train_data(self, seq_len=30, normalise=True):
        '''
    ...
    Get Afternoon data
    '''
    def get_afternoon_train_data(self, seq_len=30, normalise=True):
        '''
        Create x, y train data windows
        Warning: batch method, not generative, make sure you have enough memory to
        load data, otherwise use generate_training_window() method. 
    ...
    '''
    def next_afternoon_batch(self, seq_len = 30, batch_size = 200):
        start, end = self.afternoon_data[self.morning_index]
        self.afternoon_index = (self.afternoon_index+1)%len(self.afternoon_data)
    ...
    def normalise_windows(self, window_data, single_window=False):
        ...
        for window in window_data:
            ...
            for col_i in range(window.shape[1]):
                normalised_col = [(float(p)  - means[col_i])/stds[col_i] for p in window[:, col_i]]
